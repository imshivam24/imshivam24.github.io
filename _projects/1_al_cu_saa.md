---
layout: page
title: Al-Cu single-atom alloys for CO/CO₂ reduction
description: Kinetics of acidic CO₂RR to C1 products on Al-Cu single-atom alloys, from DFT to microkinetic selectivity trends.
img: assets/img/Chemsuschem_TOC.png
importance: 1
category: work
related_publications: true
---

Al-Cu single-atom alloys are thermodynamically attractive for electrochemical CO and CO₂ reduction, but thermodynamics alone doesn't tell you which product actually forms. This project maps out the kinetics: using first-principles calculations on Al-Cu(111) and the stepped Al-Cu(211) facet, I traced the competing CHO, COH, and HCOO reaction pathways to C1 products, backed by ab initio molecular dynamics to check catalyst and interfacial-water stability.

Feeding those barriers into a microkinetic model reveals potential- and pH-dependent selectivity: CO₂ reduction favors formic acid across most conditions, while CO reduction pushes toward methanol at higher pH. The rate-determining step for methane formation — dehydroxylation of a COH intermediate — turns out to be facet-sensitive, with a noticeably lower barrier on the stepped surface. The result is a mechanistic picture of how potential, pH, and surface structure together steer selectivity on this catalyst class.

{% cite chaturvedi2026hydrocarbon %}
